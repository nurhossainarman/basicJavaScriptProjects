...
let cities = new Array(); //Array of cities
timeZones.forEach(function(element){
  const zone = element.split("/");
  cities.push({city:`${zone[zone.length-1]}`, timezone:`${element}`});
})
...
function driver (timeZone){ //Changing the innerHtml of .element#time
    ...
    document.getElementById("city").innerHTML =  arr[arr.length-1];
...
driver('Asia/Dhaka')
setInterval(() => {driver('Asia/Dhaka')}, 1000);
